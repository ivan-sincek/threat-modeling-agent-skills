...
3. Systematically identify and document business processes using the schema defined in the `Output - Business Impact Analysis` section.

4. Order business processes by criticality rating.

### Stage 2 - Define the Technical Scope

1. Define the technical scope of the application by systematically identifying the following elements:
...
2. Systematically document each identified element using its corresponding schema defined in the `Output - Technical Scope` section.

3. Sort elements alphabetically by name.
...
2. For each diagram, incorporate all relevant elements from the previously defined technical scope to fully represent the specific use case, including:
...
1. For each previously identified threat, incorporate all relevant elements to construct an attack tree with the following swimlanes:

	| <!-- Key --> | <!-- Value --> |
	| --- | --- |
	| **Threat Actors** | `TA-#: Name<br><br>Motive` |
...
See the example output in `examples/pasta_threat_model.md`.

Do not add or modify elements or formatting.

Ensure each table follows the defined schema, including key names, ordering, orientation, and value formatting. Use `N/A` when a value cannot be determined.
...
#### Step 4.1 - Trust Boundaries

| ID | Name | Type | Description |
| --- | --- | --- | --- |
| `TB-#` | --- | `External` / `DMZ` / `Internal` | --- |

#### Step 4.2 - System Components
...
#### Step 4.8 - Technologies and Dependencies
...
| --- | --- | --- | --- | --- | --- |
| `IF-#` | --- | `Cloud` / `Hybrid` / `On-Premises` | `Container` / `VM` / `Bare Metal` / `Other` | `TB-#` | --- |

#### Step 4.10 - Threat Actors
...
- Add a heading in the format `Use Case #: Name`.
...
3. Represent elements using canonical and distinct nodes in the format `XX#@{ shape: ..., label: "XX-#: Name" }`.
4. Label data flows using canonical, explicit, and concise noun phrase names in the format `#. Name`.
5. Style data flows using canonical, distinct, and high-contrast colors in the format `linkStyle # stroke: ..., stroke-width: 3px`.

### Step 6 - Threat Details
